...
Transformer 구조를 자주 참조하며, 그 차원의 일반적인 용어를 사용합니다. Transformer 레이어의 입력 및 출력 차원 크기를 $d_{\text{model}}$이라고 부릅니다. 우리는 자기 주의 모듈의 쿼리/키/벨류/아웃풋의 투영(입력 데이터를 다른 차원이나 공간으로 변환하는 과정) 행렬을 각각 $W_q, W_k, W_v, W_o$로 지칭합니다. $W$ 또는 $W_0$는 사전 학습된 가중치 행렬을, $\Delta W$는 적응 중에 누적된 그라디언트 업데이트를 나타냅니다. LoRA 모듈의 랭크는 $r$로 표시됩니다.

# [추가 설명]

- **내재적 공간**: 내재적 공간은 모델이 학습 데이터에 과도하게 적응하여, 학습 데이터 이외의 데이터에 대한 일반화 능력이 떨어질 수 있는 현상을 설명하는 개념입니다. 이는 모델이 실제로 저차원의 내재적 공간에 존재할 수 있다는 것을 의미합니다[1](https://wandb.ai/sauravmaheshkar/Intrinsic-Dimensions/reports/What-Are-Intrinsic-Dimensions-The-Secret-Behind-LoRA--Vmlldzo2MDcxMDc5)[3](https://arxiv.org/abs/2106.09685).
- **저차원**: 이는 모델이 학습 데이터에 대한 최적의 성능을 달성하기 위해 필요한 최소한의 매개변수 수를 나타냅니다. 즉, 모델이 실제로 전체 매개변수 공간 중에서 일부만을 사용하여 학습을 수행할 수 있다는 것을 의미합니다
- **랭크:** 행렬에서 중요한 정보를 담고 있는 차원의 수를 말합니다. 예를 들어, 우리가 한 장의 이미지를 여러 픽셀로 표현하지만, 그 중 몇 개의 주요 특징만으로 이미지를 잘 설명할 수 있는 것과 같습니다.
- **내재적 랭크:** 이와 유사하게, 모델이 적응할 때 필요한 가중치 변화는 전체 파라미터가 아니라 일부 중요한 파라미터들만 변하면 된다는 것을 의미합니다.

# [랭크 분해에 대한 추가 설명]

**랭크 분해**를 통해 중요한 부분을 찾아내고, 기존 가중치 행렬을 분해한 후 일부 웨이트만 업데이트한다는 설명은 정확한 방향입니다. LoRA에서는 **기존 모델의 가중치**를 유지하면서, 새로운 작업에 맞춰서 **랭크 분해**를 사용해 변화가 필요한 부분만 학습하게 됩니다.

**랭크** 자체는 데이터나 학습 과정에서 **중요한 정보**를 압축하는 개념이지만, 어디가 중요한지 미리 알 수 있는 것은 아닙니다. 이 과정은 학습을 통해 자동으로 발견되는 부분입니다.

LoRA에서 말하는 **랭크 분해**는 기존에 학습한 가중치 행렬 $W$의 변화(즉, 새로운 작업에 필요한 정보)를 더 작은 차원으로 압축해서 표현하는 방식입니다.

구체적으로 말하자면:

1. **초기화**: 행렬 $A$와 $B$는 초기에는 무작위로 설정됩니다. 즉, 처음에는 어떤 특정한 행이나 열이 "중요하다"고 미리 정해진 것이 아닙니다. 학습이 시작되면, 모델은 데이터에 따라 중요한 방향(즉, 중요한 차원)을 찾아서 학습하게 됩니다.
2. **랭크의 의미**: 여기서의 **랭크**는 저차원의 공간으로 변환하는 데 필요한 차원의 수를 의미합니다. 예를 들어, $rank=4$라고 하면, 전체 128x128의 공간에서 4개의 중요한 방향을 찾아내 그 부분만 학습하겠다는 의미입니다. 이는 모델이 데이터를 통해 찾아내며, 중요한 정보가 담긴 부분을 학습하는 동안 자동으로 결정됩니다.
3. **B와 A의 역할**:
    - **B**는 고차원(128차원)에서 저차원(4차원)으로 정보를 압축하는 역할을 합니다.
    - **A**는 그 저차원의 정보를 다시 고차원(128차원)으로 확장하는 역할을 합니다.
    즉, $W$를 직접 학습하는 대신 $B$와 $A$를 통해 저차원의 정보만을 학습하고, 이를 다시 고차원 공간에서 사용할 수 있게 합니다.

### 왜 $B$와 $A$가 사용되는가?

- 일반적인 가중치 행렬 $W$가 128x128 차원일 경우, 이를 직접 학습하려면 **16,384개의 파라미터**를 학습해야 합니다.
- 그러나 LoRA에서는 이를 두 개의 작은 행렬로 나눕니다:
    - $B$는 128x4 크기, $A$는 4x128 크기입니다.
    - 따라서 학습할 파라미터의 수는 **512(128x4) + 512(4x128) = 1,024개**만 있으면 충분합니다.

이는 전체 파라미터 수를 크게 줄이면서도 중요한 정보(저차원의 내재적 랭크)를 학습하는 방식입니다.

### 학습 과정을 다시 정리하면:

1. **초기화**: $B$와 $A$는 무작위로 초기화됩니다. 이 시점에서는 아직 어떤 정보가 중요한지 모릅니다.
2. **학습**: 모델이 데이터를 학습하면서, 저차원 행렬 $B$와 $A$를 최적화하여, $\Delta W = BA$라는 형태로 중요한 가중치 변화를 학습합니다.
3. **결과**: 학습이 완료되면, 모델은 새로운 작업에 맞춰 중요한 정보(랭크가 낮은 저차원의 정보)를 학습하게 됩니다.

이 과정에서 **중요한 정보가 어디에 있는지**는 학습 과정에서 자연스럽게 결정되는 부분입니다.

### 랭크를 설정하는 기준:

1. **모델의 크기**: 모델이 클수록 더 많은 정보가 필요할 수 있습니다. 큰 모델일수록 높은 랭크가 필요할 가능성이 있습니다. 예를 들어, GPT-3 175B와 같은 대형 모델에서는 낮은 랭크(예: 1~4)로도 충분할 수 있지만, 더 작은 모델에서는 더 높은 랭크가 필요할 수 있습니다.
2. **작업의 복잡성**: 수행할 작업이 복잡할수록 더 많은 정보가 필요하므로 더 높은 랭크가 필요할 수 있습니다. 간단한 작업은 낮은 랭크로도 충분히 적응할 수 있지만, 복잡한 자연어 처리 작업이나 예측 작업에서는 더 높은 랭크가 필요할 수 있습니다.
3. **실험을 통한 최적화**: 일반적으로 랭크는 실험을 통해 결정됩니다. 여러 값을 시도하면서 성능을 비교해 보고 최적의 값을 선택합니다. 예를 들어, 랭크 1부터 시작해 4, 8, 16 등의 값을 테스트하면서 성능과 효율성을 평가할 수 있습니다.

### 랭크 설정의 트레이드오프:

- **낮은 랭크**: 랭크가 낮으면 학습할 파라미터가 적기 때문에 메모리와 계산 비용이 줄어듭니다. 하지만 필요한 정보가 충분히 학습되지 않으면 성능이 저하될 수 있습니다.
- **높은 랭크**: 랭크가 높으면 더 많은 정보를 학습할 수 있기 때문에 성능이 더 좋을 수 있지만, 메모리와 계산 비용이 증가합니다.

### 예시: 랭크 선택을 위한 실험

논문에서는 GPT-3 175B를 예로 들면서, **랭크 1~2** 정도로도 충분히 좋은 성능을 얻을 수 있음을 보여주고 있습니다. 하지만 이는 GPT-3와 같은 대형 모델에서만 가능한 결과일 수 있습니다. 더 작은 모델이나 다른 작업에서는 다른 최적의 랭크 값이 필요할 수 있습니다.

결론적으로, **랭크의 수는 실험을 통해 결정되며**, 특정 작업과 모델에 맞게 조정하는 것이 중요합니다.

### 핵심 아이디어:

1. **기존 가중치 고정**: 사전 학습된 모델의 가중치(예: GPT-3)를 고정합니다. 즉, 이 가중치들은 새로운 작업을 학습할 때 변경되지 않습니다.
2. **랭크 분해 행렬 학습**: 가중치의 변화 부분(즉, 새로운 작업에서 필요한 부분)을 **저차원의 랭크 분해 행렬** $A$와 $B$로 표현하고, 이 두 행렬만 학습합니다.
    - 여기서 **랭크 분해**는 $W + \Delta W = W + BA$로 표현됩니다. $W$는 사전 학습된 고정된 가중치 행렬이고, $\Delta W = BA$는 학습할 부분입니다.
    - $B$는 저차원의 행렬이고, $A$는 그에 따른 변화를 나타내는 행렬입니다. 이 둘만 학습하면, 전체 모델의 가중치 업데이트가 필요 없습니다.
...
# 기존에 사전 학습된 가중치 (고정된 W)
W_pretrained = torch.randn(128, 128)  # 기존 모델의 가중치 행렬
W_pretrained.requires_grad = False    # 고정된 가중치 (학습 안됨)

# LoRA에서 학습할 저차원 랭크 분해 행렬 A, B
rank = 4  # 저차원 랭크
A = torch.randn(rank, 128, requires_grad=True)  # 학습할 A
B = torch.randn(128, rank, requires_grad=True)  # 학습할 B

# 입력 벡터 x
x = torch.randn(128)

# W_pretrained와 랭크 분해 행렬 BA로 모델을 적용
def forward(x):
    # 고정된 W_pretrained와 학습 가능한 BA를 더하여 계산
    W_updated = W_pretrained + torch.matmul(B, A)
    return torch.matmul(W_updated, x)
...
for epoch in range(100):
    optimizer.zero_grad()
    output = forward(x)
    loss = torch.mean(output)  # 가상의 손실 함수
    loss.backward()
    optimizer.step()
...
1. **W_pretrained**: 사전 학습된 가중치 행렬로, 고정된 상태에서 학습하지 않습니다 (`requires_grad=False`).
2. **A와 B**: 학습 가능한 저차원 행렬들로, 이 두 행렬의 곱 $BA$가 가중치 업데이트를 담당합니다. 즉, **W_pretrained** + **BA**로 새로운 작업에 맞춘 모델을 구성합니다.
3. **Optimizer**: 이 예시에서는 Adam 옵티마이저를 사용해 **A**와 **B**만 학습합니다.

이렇게 하면 전체 가중치를 다시 학습하지 않고도, 필요한 부분(작업에 따라 중요한 저차원 정보)만 학습할 수 있습니다.
...
전체 파인튜닝에서, 모델의 모든 파라미터가 사전 학습된 가중치 $\Phi_0$로 초기화되고, 다운스트림 작업에 맞게 그 모든 파라미터가 업데이트됩니다. 그 목표는 주어진 문맥(입력) $x$와 그에 대한 출력 $y$의 조건부 확률을 최대화하는 것입니다.

### 첫 번째 수식:

$\max_{\Phi} \sum_{(x,y) \in Z} \sum_{t=1}^{|y|} \log P_{\Phi}(y_t | x, y_{<t})$

- $\max_{\Phi}$: 사전 학습된 모델의 가중치 $\Phi$를 최적화하려는 목표입니다.
- $\sum_{(x,y) \in Z}$: 모든 학습 데이터 $(x, y)$ 쌍에 대해 합을 구합니다. 여기서 $Z$는 학습 데이터셋입니다.
- $\sum_{t=1}^{|y|}$: 출력 문장 $y$의 각 시간 단계 $t$에 대해 합을 구합니다. 즉, 각 토큰에 대해 조건부 확률을 계산합니다.
- $\log P_{\Phi}(y_t | x, y_{<t})$: 주어진 입력 $x$와 그 이전 토큰들 $y_{<t}$을 바탕으로 $y_t$가 등장할 확률을 모델의 가중치 $\Phi$로 계산한 후, 그 로그 확률 값을 취한 것입니다. 즉, 모델이 다음 토큰 $y_t$를 예측하는 정도를 측정합니다.

이 수식은 **조건부 언어 모델링 목표**를 나타냅니다. 즉, 주어진 문맥과 그 이전 단어들이 주어졌을 때, 다음 단어의 확률을 최대화하려고 합니다.

### 2. **파라미터 효율성 문제**

전체 파인튜닝의 단점은 다운스트림 작업별로 새로운 파라미터 집합 $\Delta \Phi$를 학습해야 한다는 것입니다. 이 $\Delta \Phi$는 사전 학습된 파라미터 $\Phi_0$와 같은 크기이기 때문에, 매우 큰 모델(GPT-3 같은 1750억 개의 파라미터가 있는 모델)의 경우, 여러 파인튜닝된 모델을 저장하고 배포하는 것이 비효율적입니다.

### 3. **파라미터 효율적인 접근 (Parameter-Efficient Approach)**

논문에서 제안된 방법은 **파라미터 효율적인** 방식입니다. 여기서 각 작업마다 업데이트되는 파라미터 증가량 $\Delta \Phi$를 더 작은 파라미터 집합 $\Theta$(Theta)로 인코딩합니다.

### 두 번째 수식:

$\max_{\Theta} \sum_{(x,y) \in Z} \sum_{t=1}^{|y|} \log P_{\Phi_0 + \Delta \Phi(\Theta)}(y_t | x, y_{<t})$

- $\max_{\Theta}$: 이제 모델을 직접적으로 **파인튜닝**하는 대신, 훨씬 작은 파라미터 집합 $\Theta$를 최적화합니다.
- $P_{\Phi_0 + \Delta \Phi(\Theta)}$: 사전 학습된 가중치 $\Phi_0$ 고정되어 있고, $\Delta \Phi(\Theta)$라는 작은 파라미터 세트만 학습합니다. 즉, $\Delta \Phi$를 $\Theta$라는 더 작은 파라미터로 표현하여 이를 최적화하는 방식입니다.

### 요약

1. **전체 파인튜닝**에서는 모델의 모든 파라미터 $\Phi$가 최적화 대상이 됩니다. 각 다운스트림 작업마다 별도의 파라미터 $\Delta \Phi$가 학습되어야 하며, 이 파라미터는 사전 학습된 모델과 같은 크기를 가집니다.
2. 그러나 논문에서 제안한 방식에서는 **작업별 파라미터 증가량** $\Delta \Phi$를 훨씬 작은 파라미터 $\Theta$로 압축해서 표현하고, 이를 최적화합니다. 이렇게 하면 저장 및 계산의 부담을 크게 줄일 수 있습니다.

즉, **작은 파라미터 집합** $\Theta$만 최적화하여 사전 학습된 모델을 효율적으로 적응시키는 방법이 LoRA의 핵심입니다.
...
### 1. 어댑터 레이어 (Adapter Layer)란?

어댑터 레이어는 사전 학습된 대형 언어 모델(예: GPT-3, BERT 등)을 특정한 다운스트림 작업에 맞춰 미세 조정(fine-tuning)할 때 **모든 파라미터를 학습하는 대신, 일부 추가적인 작은 파라미터만 학습하는 방식**입니다. 이를 통해 사전 학습된 모델의 대부분의 가중치는 고정된 상태로 유지하고, 어댑터 레이어에만 새로운 작업에 필요한 변화를 학습시킵니다.

어댑터 레이어는 모델의 기존 레이어 사이에 삽입되며, 이 레이어는 매우 작은 병목 차원을 통해 입력을 처리합니다. 작은 차원에서 학습하므로 메모리 사용량을 줄일 수 있고, 이를 통해 모델의 효율성을 높입니다.

### **LayerNorm (Layer Normalization)의 역할**:

LayerNorm은 **모델의 각 레이어에 있는 뉴런들의 출력을 정규화**하는 기법입니다. 이를 통해 학습이 더 안정적이고 빠르게 이루어지도록 돕습니다. 특히 대형 신경망에서는 각 레이어의 출력이 상이한 범위에 있을 수 있기 때문에, LayerNorm은 이러한 출력을 일정한 범위로 정규화하여 **학습 과정에서의 기울기 폭발 및 소실**을 방지합니다.

### 2. **계층을 제거하거나 다중 작업 설정을 활용한다는 의미**

- **계층을 제거한다**: 어댑터 레이어를 추가하면 그만큼 모델의 깊이가 늘어나고 추가적인 계산이 필요합니다. 따라서 불필요한 계층을 제거함으로써 전체적인 연산량과 지연 시간을 줄일 수 있습니다.
- **다중 작업 설정을 활용한다**: 다중 작업(Multi-Task) 설정은 **여러 작업을 동시에 학습**하는 방식입니다. 여러 작업에 대해 별도의 어댑터 레이어를 두는 대신, **하나의 모델이 여러 작업에 대해 공통된 어댑터를 사용**할 수 있습니다. 이렇게 하면 중복된 작업을 줄이고 효율성을 높일 수 있습니다. 하지만 어댑터 레이어의 추가적인 계산 자체는 여전히 피할 수 없다는 점에서 한계가 있습니다.

### 3. **어댑터 레이어가 매우 작은 병목 차원을 가지고 있다는 의미**

"병목 차원"이라는 용어는 **어댑터 레이어가 입력 데이터를 처리하는 중간에서 매우 작은 차원으로 축소하여 연산하는 과정**을 의미합니다.

- **병목 차원**이 작다는 것은 어댑터 레이어가 원래 모델보다 훨씬 적은 수의 파라미터를 사용한다는 의미입니다. 예를 들어, 원래 모델의 차원이 1024라고 하면, 어댑터 레이어에서는 이를 64 또는 32와 같은 작은 차원으로 축소하여 연산합니다.
- 이를 통해 **계산 비용(FLOP 수)**를 줄일 수 있습니다. 하지만 이 작은 병목 차원으로 인해 모델 성능이 제한될 수 있습니다.

### 4. **어댑터 레이어는 순차적으로 처리하는데, 이게 무슨 뜻이고 모델을 샤딩할 때 더 심각해진다는 의미는?**

- **순차적으로 처리**한다는 말의 의미:
어댑터 레이어는 **각각의 트랜스포머 블록에 삽입**되어 순차적으로 연산이 이루어집니다. 예를 들어, 입력 데이터가 트랜스포머 레이어를 통과하면서 어댑터 레이어도 함께 거치게 되며, 이 과정은 하나의 블록이 끝난 후 다음 블록이 처리됩니다.
    
    대형 신경망에서는 **하드웨어 병렬 처리**를 통해 지연 시간을 줄이는 것이 매우 중요합니다. 그러나 어댑터 레이어가 순차적으로 처리되면, 병렬 처리가 어려워지고 지연 시간이 늘어납니다. 즉, 어댑터 레이어가 추가될 때마다 계산이 점점 더 느려지는 문제가 발생합니다.
    
- **모델을 샤딩할 때 심각해진다**는 의미:
"모델 샤딩"이란, 매우 큰 모델을 여러 개의 GPU나 서버에 **나누어(shard)** 분산해서 처리하는 것을 말합니다. 모델을 여러 부분으로 나누어 병렬로 처리할 수 있지만, 어댑터 레이어가 추가된 경우 이 **동기화**가 필요해지면서 더 복잡해집니다.
    - 예를 들어, **AllReduce**나 **Broadcast** 같은 동기화 작업은 여러 GPU가 서로 계산 결과를 공유하고 결합할 때 사용하는 기법입니다. 어댑터 레이어가 추가되면 이러한 동기화 작업이 더 자주 발생하게 되어 성능 저하가 심화됩니다.
...
프리픽스 튜닝은 대형 언어 모델(예: GPT-3)에서 **모델의 모든 파라미터를 업데이트하지 않고** 입력 프롬프트 앞에 추가적인 "프리픽스 토큰"을 삽입해 이들의 임베딩을 학습시키는 방식입니다. 이 방식은 파라미터 수를 줄이고 **모델을 효율적으로 튜닝**할 수 있도록 고안되었습니다. 프리픽스 튜닝은 프롬프트에 **학습 가능한 임베딩 토큰**을 추가하여, 특정 작업에 맞게 모델이 더 잘 응답할 수 있도록 조정하는 방법입니다.

### 2. **프리픽스 튜닝 시 훈련 가능한 파라미터가 증가하고 성능이 일관되지 않게 변하는 이유**

- **훈련 가능한 파라미터 증가**: 프리픽스 튜닝에서는 기존 모델의 모든 파라미터를 고정시키고, 입력 데이터 앞에 새로운 프리픽스 토큰을 삽입해 이들의 임베딩을 학습시킵니다. 이 프리픽스 토큰에 대한 임베딩은 추가적인 학습이 필요한 파라미터이기 때문에, **프리픽스 토큰 수가 많아질수록 학습해야 하는 파라미터의 수가 늘어납니다**. 특히, 이 프리픽스 토큰 수가 늘어나면 모델이 학습해야 하는 파라미터 수가 급증할 수 있습니다.
- **성능이 일관되지 않게 변하는 이유**: 프리픽스 튜닝은 **프리픽스 토큰의 수**와 **그 위치**에 따라 성능이 크게 달라질 수 있습니다. 만약 적절하지 않은 토큰 수나 위치가 선택되면, 학습 효과가 오히려 감소하고 성능이 저하될 수 있습니다. 더불어, 프리픽스 토큰이 모델의 기존 시퀀스 길이 일부를 차지하기 때문에 모델이 원래 학습한 정보와 충돌할 수 있습니다.

### 3. **시퀀스 길이의 일부를 적응에 예약한다는 것의 의미와 시퀀스 길이가 줄어드는 이유**

- **시퀀스 길이의 일부를 적응에 예약한다는 것**: 프리픽스 튜닝은 모델 입력 앞에 **프리픽스 토큰**을 추가합니다. 이 토큰들은 모델이 작업에 맞게 튜닝될 수 있도록 학습됩니다. 하지만 이 프리픽스 토큰이 모델이 처리할 수 있는 시퀀스 길이 일부를 차지하게 됩니다.
- **다운스트림 작업의 시퀀스 길이 감소**: 프리픽스 토큰이 시퀀스에 추가되면, 실제로 처리할 수 있는 **작업 관련 데이터의 시퀀스 길이**가 줄어듭니다. 예를 들어, GPT-3가 한 번에 처리할 수 있는 시퀀스 길이가 1024 토큰이라고 할 때, 프리픽스 튜닝을 위해 50개의 프리픽스 토큰을 추가하면 남은 974 토큰만 실제 작업에 사용할 수 있습니다. 이로 인해 **다운스트림 작업을 처리할 수 있는 시퀀스 길이가 줄어들게 되는 것**입니다.

### 4. **AdapterL과 AdapterH는 무엇을 지칭하는지?**

- **AdapterL과 AdapterH**는 **어댑터 레이어의 두 가지 변형**을 지칭합니다.
    - **AdapterL**: Lin et al. (2020)이 제안한 어댑터 구조로, 각 트랜스포머 블록마다 **하나의 어댑터 레이어**를 포함하며, **LayerNorm**을 추가한 방식입니다. 이 방식은 효율성을 높이기 위해 최소한의 구조만을 추가하는 방식입니다.
    - **AdapterH**: Houlsby et al. (2019)이 제안한 구조로, **각 트랜스포머 블록마다 두 개의 어댑터 레이어**가 포함됩니다. 더 복잡한 구조이지만, 더 많은 파라미터를 학습할 수 있어 작업에 대한 적응력이 높아질 수 있습니다.

이 두 방식 모두 사전 학습된 대형 모델에 소규모 어댑터 레이어를 삽입하여 새로운 작업에 맞게 적응할 수 있도록 설계되었으며, **학습된 파라미터 수와 효율성에서 차이**가 있습니다.
...
우리는 그림 1에서 우리의 재매개변수를 설명합니다. A는 랜덤 가우시안 초기화를 사용하고, B는 0으로 설정하여, 훈련 시작 시 ∆W = BA가 0이 되게 합니다. 그런 다음 우리는 ∆Wx에 대해 α/r로 스케일링하며, 여기서 α는 r에서의 상수입니다. Adam으로 최적화할 때, α를 조정하는 것은 학습률을 조정하는 것과 거의 동일하며, 초기화를 적절히 스케일링하면 α를 처음 시도하는 r로 단순히 설정하고 조정하지 않습니다. 이 스케일링은 r을 변경할 때 과도한 하이퍼파라미터 조정을 줄이는 데 도움이 됩니다(Yang & Hu, 2021).
...
"내재적 차원"은 데이터나 모델이 본래 가지고 있는 **정보의 복잡도를 측정**하는 척도입니다. 즉, 높은 차원을 가진 데이터라도 그 정보가 실제로 몇 개의 주요한 차원에 집중되어 있을 수 있습니다. 예를 들어, 어떤 데이터가 100차원의 공간에 있더라도, 그 데이터가 실제로 3차원으로 충분히 설명될 수 있다면, 이 데이터는 내재적으로 3차원의 구조를 가진다고 말할 수 있습니다. Aghajanyan et al. (2020)의 연구에서는 **사전 학습된 언어 모델이 적은 내재적 차원만으로도 효율적으로 학습할 수 있다**는 점을 보여줍니다.

### 2. **d와 k는 각각 무엇을 의미하는지?**

- **d**: 입력 벡터의 차원을 나타냅니다. 예를 들어, 트랜스포머 모델의 경우 입력으로 들어가는 단어 임베딩의 크기가 될 수 있습니다.
- **k**: 출력 벡터의 차원입니다. 트랜스포머 모델에서 밀집층을 통과한 후 나오는 벡터의 차원입니다.

따라서 $W_0 \in \mathbb{R}^{d \times k}$ 는 **입력 벡터**의 차원이 $d$, **출력 벡터**의 차원이 $k$인 가중치 행렬을 의미합니다.

### 3. **왜 min(d, k)를 사용하는지?**

$r \ll \min(d, k)$라는 표현에서 **min**을 사용하는 이유는, **r이 d와 k 중 더 작은 값보다도 훨씬 작은 저차원이어야 한다**는 점을 강조하기 위해서입니다. 이 식은 주어진 가중치 행렬이 $d \times k$ 크기일 때, 저차원의 행렬로 분해할 때 **r**이 d와 k 둘 중 더 작은 값보다 훨씬 작아야 한다는 조건을 나타냅니다. 이는 저랭크(Low-Rank) 분해에서 **차원 축소**가 이루어지는 부분입니다.

### 4. **B와 A는 각각 무엇을 의미하는지?**

- **B**: 가중치 업데이트 행렬의 첫 번째 분해된 부분으로, $d \times r$ 크기의 행렬입니다. **입력 차원**에 해당하는 부분을 담당합니다.
- **A**: 가중치 업데이트 행렬의 두 번째 분해된 부분으로, $r \times k$ 크기의 행렬입니다. **출력 차원**에 해당하는 부분을 담당합니다.

따라서, 원래의 가중치 업데이트를 **저차원 공간**으로 분해하여 처리하는데, **B**와 **A**는 이러한 분해 과정에서 학습되는 저차원 행렬들입니다.

### 5. **∆W에 대해** $\alpha/r$**로 스케일링한다고 했는데 현재 공식에 반영이 안된 것이 맞는지? 그리고 왜 이 스케일링이 필요한지?**

맞습니다. 현재 제시된 공식에서는 $\alpha/r$ 스케일링이 포함되어 있지 않습니다. 이는 훈련 과정에서 적용되는 스케일링입니다. 스케일링은 **훈련 과정에서 학습의 안정성**을 높이기 위해 사용됩니다.

스케일링은 **훈련 과정에서 안정성**을 높이는 역할을 합니다. $\alpha/r$ 스케일링을 사용하면, **랭크 r**이 클수록 학습 과정에서의 변화가 지나치게 커지는 것을 방지할 수 있습니다. 작은 랭크에서 학습된 값이 적절한 크기로 유지되도록 하기 위해서, **r에 비례하여 업데이트를 축소**하는 것이 필요합니다. 이를 통해 **과도한 업데이트**가 일어나지 않고, 학습률을 일관되게 유지할 수 있습니다. 공식에 포함되지 않은 이유는, 이 스케일링이 **훈련 중 적용되는 기법**이기 때문입니다. 즉, 순방향 패스에서는 스케일링이 포함되지 않지만, **훈련 시에는 적용**됩니다.

### 6. **MLP란 무엇인지?**

MLP는 **Multi-Layer Perceptron**의 약자로, 다층 퍼셉트론을 의미합니다. 이는 인공 신경망에서 가장 기본적인 형태의 **Fully Connected Neural Network**입니다. MLP는 여러 개의 입력 노드를 받아 여러 층의 퍼셉트론을 통과시키면서 최종적으로 출력 값을 예측합니다.

MLP는 일반적으로 입력층, 은닉층(또는 여러 개의 은닉층), 그리고 출력층으로 구성됩니다. 각 층의 노드는 이전 층의 모든 노드와 연결되어 있으며, 각 연결에는 가중치(weight)가 부여됩니다. MLP는 주로 구조가 단순하지만, 문제의 복잡성에 따라 여러 층과 뉴런을 추가할 수 있습니다.

### 7. **LoRA가 원래의 모델을 훈련하는 것과 수렴한다는 것은?**

**랭크**가 **전체 차원수에 가까워질수록** LoRA는 원래 모델을 훈련하는 것과 매우 유사하게 작동합니다.

- **LoRA**는 저차원에서 적응할 수 있도록 설계된 방법으로, 파라미터의 수를 줄이고 **저차원 랭크 r**만으로 학습을 수행합니다.
- 하지만 **r을 계속 증가**시키면 결국에는 **풀랭크**에 가까워집니다. 즉, 저차원의 제한이 없어진 상태에서, LoRA는 더 이상 저차원으로 학습하지 않고, **기존의 전체 모델을 훈련하는 것과 동일하게** 동작하게 됩니다.
- **수렴**한다는 표현은, **랭크가 증가하면서** LoRA 방식이 점점 **원래의 파인튜닝 방식과 동일한 결과를 제공한다는 의미**입니다.

따라서 **랭크가 작을수록** LoRA는 파라미터 수가 적어 효율적이지만, **랭크가 커질수록** 전체 모델을 학습하는 것과 비슷해지며, 원래의 모델 훈련 방식과 **결과적으로 유사한 성능**을 가지게 됩니다.

---

### 8. **BA를 빼고 B'A'를 더하여 W0를 복구한다는 것의 의미?**

이 문장에서 말하는 바는 **LoRA를 사용하여 다양한 작업(task)을 수행하는 모델을 전환하는 과정**을 설명하는 것입니다.

### **BA를 빼고 다른 B'A'를 더하는 이유는?**

- **W0**는 **사전 학습된 모델의 고정된 가중치 행렬**입니다. 즉, 모든 다운스트림 작업에서 **공유**되는 가중치입니다.
- **BA**는 특정 **다운스트림 작업**을 위한 **저차원 업데이트**를 나타냅니다. 작업 A에 대한 **변경(업데이트)**을 BA로 적용한 상태라고 생각하면 됩니다.

따라서,

- **작업 A**에서 **작업 B**로 전환할 때, **W0에 적용된 BA**를 제거하고, **작업 B**에 맞는 새로운 **B'A'**를 적용해야 합니다.
- 이 과정에서 **BA를 빼고 B'A'를 더함으로써** 모델의 **W0**는 여전히 유지되지만, **특정 작업에 맞는 저차원 업데이트(B'A')**가 적용된 상태로 전환됩니다. 이를 통해 **빠른 작업 전환**이 가능해지며, **메모리 오버헤드도 매우 적습니다**.

### **구체적인 과정:**

1. **작업 A**에서: 모델의 가중치가 $W_0 + BA$인 상태로 학습합니다.
2. **작업 B**로 전환할 때: 기존의 **BA** 업데이트를 제거한 후, 새로운 **B'A'**를 더하여 $W_0 + B'A'$ 상태로 만듭니다.
    - 이렇게 하면 **기존의 W0 가중치**는 고정된 채, 새로운 **작업 B에 맞는 업데이트(B'A')**만 적용되어 새로운 작업을 처리할 수 있습니다.

이 방법은 **빠른 작업 전환**을 가능하게 하면서도 **메모리 효율성**을 유지할 수 있는 큰 장점이 있습니다.

---

### 4.2 트랜스포머에 LORA 적용하기

원칙적으로, 우리는 신경망의 가중치 행렬 중 일부에 LoRA를 적용하여 훈련 가능한 파라미터 수를 줄일 수 있습니다. 트랜스포머 아키텍처에는 자기-주의 모듈에서 네 개의 가중치 행렬($W_q, W_k, W_v, W_o$)과 MLP 모듈에서 두 개의 가중치 행렬이 있습니다. 우리는 $W_q$ (또는 $W_k, W_v$)를 단일 행렬로 취급합니다. 이 행렬은 차원이 $d_{\text{model}} \times d_{\text{model}}$이지만, 출력 차원은 일반적으로 어텐션 헤드로 나뉩니다. 우리의 연구는 단순함과 파라미터 효율성을 위해 다운스트림 작업에서 어텐션 가중치만 적응시키고 MLP 모듈은 동결하여 훈련하지 않습니다. 트랜스포머에서 다양한 유형의 어텐션 가중치 행렬 적응에 대한 연구는 섹션 7.1에서 다룹니다. MLP 레이어, LayerNorm 레이어 및 바이어스 적응에 대한 실증적인 연구는 추후 작업으로 남겨둡니다.
...
## 왜 VRAM 사용량이 2/3까지 줄어드는지:

1. **기본 메모리 사용 구조**:
    - **일반적인 훈련 시**, 옵티마이저는 모델의 **모든 파라미터**에 대해 **추가 메모리**를 사용합니다. 예를 들어, Adam 옵티마이저는 **1차 및 2차 모멘트**(이전 기울기 정보와 관련된 추가 데이터)를 추적해야 하므로, 각 파라미터에 대해 추가 메모리가 필요합니다.
    - 파라미터 하나당 **3배**의 메모리가 필요하다는 것을 생각하면 됩니다:
        1. **모델 가중치** 자체.
        2. 옵티마이저의 **1차 모멘트(이전 기울기)**.
        3. 옵티마이저의 **2차 모멘트(기울기 변화의 제곱값)**.
2. **LoRA에서의 메모리 절감**:
    - LoRA는 **대부분의 파라미터(W0)를 고정**하고, **소수의 저랭크 행렬(A와 B)**만 업데이트합니다. 이때 **W0에 대한 옵티마이저 상태**는 저장할 필요가 없고, 오직 **A와 B**만 추적하면 됩니다.
    - 즉, 파라미터 수가 **훨씬 줄어든** 저랭크 행렬(A와 B)에 대해서만 옵티마이저 상태를 관리하면 됩니다. 이를 통해 **전체 메모리 사용량**이 크게 줄어듭니다.
3. **왜 2/3인가?**:
    - **모델 가중치** 자체는 여전히 메모리에 저장해야 합니다. LoRA는 **학습 중에 옵티마이저 상태(1차 및 2차 모멘트)를 줄이는 방식**이므로, **최대 2/3**까지 줄어들 수 있습니다.
    - **1/3은 여전히 모델 가중치**와 관련된 메모리 사용량으로 남아 있으며, 이것은 고정되어 있는 파라미터(W0)를 저장하는 데 사용됩니다. 따라서 이를 초과하여 더 많이 줄일 수는 없습니다.
    
    구체적으로:
    
    - **기존 훈련**에서는 파라미터 하나당 3배의 메모리(모델 가중치 + 옵티마이저 상태) 사용.
    - **LoRA 훈련**에서는 대부분의 파라미터(W0)가 고정되기 때문에, 옵티마이저 상태를 저장할 필요가 없어 **옵티마이저 상태에 필요한 2배의 메모리 사용이 줄어듦**.

## **MLP 모듈은 동결하여 훈련하지 않는 이유**

LoRA는 트랜스포머의 **Self-Attention** 모듈에서만 가중치를 적응시키는 방식을 선택합니다. 이는 **단순함**과 **파라미터 효율성**을 위해 이루어진 선택입니다.

- 트랜스포머의 **Self-Attention 모듈**은 중요한 정보를 추출하고 여러 어텐션 헤드를 통해 입력 데이터 간의 관계를 학습하는 핵심 부분입니다.
- 반면, **MLP(다층 퍼셉트론) 모듈**은 **Self-Attention** 모듈이 추출한 정보를 더 깊이 처리하는 역할을 하지만, LoRA는 **어텐션 가중치만을 수정**함으로써 대부분의 파라미터를 동결하고, 적은 수의 파라미터만을 학습하는 방식으로 효율성을 높입니다.
- **MLP를 동결**하면 훈련할 파라미터의 양이 크게 줄어들기 때문에, **메모리와 계산 비용이 절감**됩니다. 또한, MLP 모듈은 다양한 작업에서도 크게 변형될 필요가 없다고 가정할 수 있어 동결해도 성능에 크게 영향을 미치지 않습니다.

## **체크포인트란 무엇이고 어떤 역할을 하는지?**

**체크포인트**(Checkpoint)는 딥러닝 모델을 **훈련하는 과정에서 모델의 가중치, 옵티마이저 상태, 학습 진행 상태 등을 저장**한 파일을 의미합니다. 훈련 중간에 모델을 중단하고 나중에 다시 시작하거나, 모델이 학습된 상태를 나중에 사용하기 위해 체크포인트를 저장합니다.

### 체크포인트의 역할:

1. **모델 저장**: 특정 시점에서 **모델의 가중치 및 상태를 저장**하여, 훈련을 다시 시작할 때 그 시점부터 이어서 훈련할 수 있습니다.
2. **모델 배포**: 학습된 모델을 배포할 때, 체크포인트를 사용하여 모델을 로드하고 추론에 사용할 수 있습니다.
3. **실험 재현성**: 모델 학습을 여러 단계로 나누어 진행하거나, 중단 후 다시 시작할 수 있기 때문에 **실험 재현성**을 보장합니다.

### 체크포인트 크기 감소:

- **LoRA의 경우**, 사전 학습된 가중치 **W0는 고정**되어 있고, 학습 중에 **저랭크 행렬(A와 B)**만 업데이트됩니다.
- 따라서 **체크포인트 파일에는 W0 전체를 저장할 필요가 없고**, 오직 **작은 저랭크 행렬 A와 B**만 저장하면 됩니다.
- 이로 인해 **체크포인트 크기가 원래의 GPT-3 175B의 350GB에서 35MB로 약 10,000배 감소**하게 되는 것입니다.
